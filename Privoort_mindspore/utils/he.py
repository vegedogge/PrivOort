...
def flatten_weights(model: nn.Cell):
    #weights = model.trainable_variables
    # params = list(model.parameters())
    params = list(model.trainable_params())
    # flat_list = [p.detach().numpy().reshape(-1) for p in params]
    flat_list = [p.asnumpy().reshape(-1) for p in params]
...
def rebuild_weights(vec, shapes, sizes):
    ...
    for shape, size in zip(shapes, sizes):
        slice_ = np.array(vec[offset:offset+size], dtype = np.float32).reshape(shape)
        out.append(np.array(slice_))
        offset += size
    return out
...
def decrypt_vector(blob, ctx):
    ...
    v.link_context(ctx)
    return np.array(v.decrypt(), dtype = np.float32)
